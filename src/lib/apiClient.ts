...
class ApiClient {
  ...
  // POST request
  async post<T = DefaultApiResponse>(endpoint: string, data?: Record<string, unknown> | FormData): Promise<T> {
  ...
  // PUT request
  async put<T = DefaultApiResponse>(endpoint: string, data?: Record<string, unknown> | FormData): Promise<T> {
    let body: string | FormData | undefined = undefined;
    if (data instanceof FormData) {
      body = data;
    } else if (data) {
  ...
  // PATCH request
  async patch<T = DefaultApiResponse>(endpoint: string, data?: Record<string, unknown> | FormData): Promise<T> {
    let body: string | FormData | undefined = undefined;
    if (data instanceof FormData) {
      body = data;
    } else if (data) {
