...
export default function DefaultInputs({ onSubmitUser }: { onSubmitUser?: (data: Record<string, unknown>) => void }) {
  ...
  const validateForm = async () => {
    ...
    if (users.some((u: Record<string, unknown>) => u.email === form.email)) {
      setSuccess("Email đã tồn tại trong hệ thống.");
      return false;
    }
    if (users.some((u: Record<string, unknown>) => u.phone === form.phone)) {
  ...
  return (
    <ComponentCard title="Thêm người dùng">
