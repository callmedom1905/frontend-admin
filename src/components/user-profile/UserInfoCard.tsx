...
const ProgressBar = ({ title, percentage, color }: { title: string, percentage: number, color: string }) => {
    return (
        <div>
            <p className="mb-1.5 text-sm font-medium text-black dark:text-white/90">{title}</p>
...
export default function UserInfoCard() {
  ...
  const projectStatus = [
      {title: "Thiết kế Web", percentage: 80, color: "bg-primary"},
      {title: "Đánh dấu Website", percentage: 72, color: "bg-red-500"},
      {title: "Trang đơn", percentage: 89, color: "bg-green-500"},
      {title: "Mẫu di động", percentage: 55, color: "bg-yellow-500"},
      {title: "API Backend", percentage: 66, color: "bg-teal-500"},
  ]
