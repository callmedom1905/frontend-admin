...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshUser = async () => {
    ...
    try {
      const userApiRes = await authAPI.getUser();
      const userData = (userApiRes as { data?: User }).data ?? null;
      if (!userData) return;
          ...
          try {
            ...
            if (parsed.role) {
              ...
              if (typeof window !== 'undefined') {
                // Fallback: User API thiếu role, đã lấy từ cookie. Nên sửa backend trả về user kèm role.
              }
            }
          } catch {}
        }
  ...
  const login = async (email: string, password: string) => {
    try {
      const dataRes = await authAPI.login(email, password);
      const data = (dataRes as { data?: LoginResponse }).data ?? null;
  ...
  const loginByPhone = async (phone: string, password: string) => {
    try {
      const dataRes = await authAPI.loginByPhone(phone, password);
      const data = (dataRes as { data?: LoginResponse }).data ?? null;
      if (!data) throw new Error("Login failed");
